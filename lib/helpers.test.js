...
describe("lib.helpers", () => {
    ...
    describe("lib.helpers.globPromise", () => {
        ...
        test("returns a promise that is fulfilled with files matching the glob pattern", async () => {
            await expect(helpers.globPromise("./test/fixtures/**/*.txt"))
                .resolves.toHaveLength(3);
            await expect(helpers.globPromise("./test/fixtures/**/*.txt"))
                .resolves.toContain("./test/fixtures/a.txt");
            await expect(helpers.globPromise("./test/fixtures/**/*.txt"))
                .resolves.toContain("./test/fixtures/more-files/b.txt");
            await expect(helpers.globPromise("./test/fixtures/**/*.txt"))
                .resolves.toContain("./test/fixtures/more-files/even more/c.txt");
        });

    });

    describe("lib.helpers.readFilePromise", () => {

        test("returns a primise that is fulfilled with the file content", () => {
            return expect(helpers.readFilePromise("test/fixtures/readfile"))
                .resolves.toEqual(Buffer.from("hello\n"));
        });

        test("returns a primise that is fulfilled with the file content", () => {
            return expect(helpers.readFilePromise("test/fixtures/not-a-file"))
