...
function javascriptExtractor(fileName, fileContent, config = DEFAULT_CONFIG.extract.javascript, isJsx = false) {  // eslint-disable-line max-len
    ...
    for (let i = 0; i < ast.tokens.length; i++) {
        ...
        if (!inFunction && !inParenthesis) {
            if (ast.tokens[i].type === "Identifier" && config.gettextFunctions.indexOf(ast.tokens[i].value) > -1) {
                inFunction = true;
            }
